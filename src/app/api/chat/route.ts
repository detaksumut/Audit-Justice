...
export async function POST(req: Request) {
  try {
    ...
    if (!userApiKey || userApiKey === 'TEST' || userApiKey.includes('masukkan_') || userApiKey.includes('your_')) {
      // MOCK MODE FOR TESTING
      await new Promise(resolve => setTimeout(resolve, 1500));
      return NextResponse.json({ reply: "Halo! Ini adalah balasan simulasi otomatis karena Anda belum memasukkan API Key di menu Pengaturan. Masukkan API Key agar saya bisa mengaudit dan membaca dokumen dengan nyata!" });
    }
